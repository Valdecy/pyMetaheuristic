...
# Function: GMO
def geometric_mean_optimizer(size = 15, iterations = 150, min_values = [-100, -100], max_values = [100, 100], epsilon = 0.001, target_function = target_function, verbose = True):
    min_values = np.array(min_values)
    max_values = np.array(max_values)
    max_vel    = 0.1 * (max_values - min_values)
    min_vel    = -max_vel
    pp         = initial_variables(size, min_values, max_values, target_function)
    pb         = np.copy(pp)
