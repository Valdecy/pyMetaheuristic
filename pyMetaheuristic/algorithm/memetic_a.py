...
def xhc(offspring, fitness, min_values = [-5,-5], max_values = [5,5], mu = 1, target_function = target_function):
    ...
    for _ in range (0, offspring.shape[0]):
        parent_1, parent_2 = roulette_wheel(fitness), roulette_wheel(fitness)
        while parent_1 == parent_2:
            parent_2 = np.random.choice(len(offspring) - 1)
        for j in range(0, offspring.shape[1] - 1):
            rand   = np.random.rand()
            rand_b = np.random.rand()                                
            if (rand <= 0.5):
...
# Function: MA
def memetic_algorithm(population_size = 5, mutation_rate = 0.1, elite = 1, min_values = [-100,-100], max_values = [100,100], eta = 1, mu = 1, std = 0.1, generations = 5000, target_function = target_function, verbose = True, start_init = None, target_value = None):    
    ...
    while (count <= generations): 
        ...
        if ((population[:,0:population.shape[1]-1].std())/len(min_values) < std):
            ...
            population = initial_variables(population_size, min_values, max_values, target_function, start_init = elite_ind)
        fitness = fitness_function(population)
