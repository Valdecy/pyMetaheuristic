...
# Lesson: Improved Whale Optimization Algorithm

# Citation: 
# PEREIRA, V. (2021). Project: Metaheuristic-Improved Whale_Optimization_Algorithm, File: i_woa.py, GitHub repository: <https://github.com/Valdecy>
    ...
    for i in range (0, offspring.shape[0]):
        ...
        for j in range(0, offspring.shape[1] - 1):
            ...
            if(i < position.shape[0] - 1):   
                offspring[i+1,j] = np.clip(((1 - b_offspring)*position[parent_1, j] + (1 + b_offspring)*position[parent_2, j])/2, min_values[j], max_values[j]) 
...
# iWOA Function
def improved_whale_optimization_algorithm(hunting_party = 5, spiral_param = 1,  mu = 1, min_values = [-5,-5], max_values = [5,5], iterations = 50, target_function = target_function):    
    count    = 0
