            ...
            for j in range(d):
                if np.random.random()<K:
                    if np.random.random()<0.5:
                        ...
                        attempted_labels[i]="artemisinin_o.self_growth_update"
                    else:
                        new_pop[i,j]=pop[i,j]+E*best_pos[j]*(-1)**t
                        attempted_labels[i]="artemisinin_o.best_growth_update"
                else:
                    new_pop[i,j]=pop[i,j]
                if np.random.random()<Fitnorm[i]:
                    ...
                    attempted_labels[i]="artemisinin_o.differential_mutation_update"
            # mutation
            for j in range(d):
                if np.random.random()<0.05:
                    new_pop[i,j]=pop[i,j]
                    attempted_labels[i]="artemisinin_o.self_reset_mutation"
                if np.random.random()<0.2:
                    new_pop[i,j]=best_pos[j]
                    attempted_labels[i]="artemisinin_o.best_reset_mutation"
            # boundary
            for j in range(d):
                if new_pop[i,j]>hi[j] or new_pop[i,j]<lo[j]:
                    new_pop[i,j]=best_pos[j]
                    attempted_labels[i]="artemisinin_o.boundary_best_repair"
