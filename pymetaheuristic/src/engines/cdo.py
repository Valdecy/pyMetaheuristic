...
class CDOEngine(PortedPopulationEngine):
    ...
    def _step_impl(self, state, pop):
        ...
        order = self._order(pop[:,-1]); b1=pop[order[0],:-1]; b2=pop[order[1],:-1] if n>1 else b1; b3=pop[order[2],:-1] if n>2 else b2
        a = 3.0-3.0*t/T
        ...
        new_pos=np.empty_like(pop[:,:-1]); operator_labels=[]
        for i in range(n):
            r1,r2=np.random.random(dim),np.random.random(dim); pa=np.pi*r1**2/(0.25*a1)-a*np.random.random(dim); c1=r2**2*np.pi
            pos_a=0.25*(b1-pa*np.abs(c1*b1-pop[i,:-1]))
            r3,r4=np.random.random(dim),np.random.random(dim); pb=np.pi*r3**2/(0.5*a2)-a*np.random.random(dim); c2=r4**2*np.pi
            pos_b=0.5*(b2-pb*np.abs(c2*b2-pop[i,:-1]))
            r5,r6=np.random.random(dim),np.random.random(dim); pc=np.pi*r5**2/a3-a*np.random.random(dim); c3=r6**2*np.pi
            pos_c=b3-pc*np.abs(c3*b3-pop[i,:-1])
            components = {"cdo.alpha_cheetah_attack_component": float(np.linalg.norm(pos_a-pop[i,:-1])), "cdo.beta_cheetah_attack_component": float(np.linalg.norm(pos_b-pop[i,:-1])), "cdo.gamma_cheetah_attack_component": float(np.linalg.norm(pos_c-pop[i,:-1]))}
            operator_labels.append(max(components, key=components.get))
            new_pos[i]=np.clip((pos_a+pos_b+pos_c)/3, self._lo, self._hi)
        new_fit=self._evaluate_population(new_pos); pop=np.hstack([new_pos, new_fit[:,None]])
