"""pyMetaheuristic src — Dispersive Fly Optimization Engine"""
...
class DFOEngine(BaseEngine):
    ...
    def step(self, state):
        ...
        evals=0
        nb=pop[pop[:,-1].argsort()][0,:]
        r=np.random.rand(self._n,self.problem.dimension)
        pop[:,:-1]=np.clip(nb[:-1]+r*(elite[:-1]-pop[:,:-1]),lo,hi)
        pop[:,-1]=self._evaluate_population(pop[:,:-1]); evals+=self._n
        rm=np.random.rand(self._n)<self._dt
        if np.any(rm):
            rv=np.random.uniform(lo,hi,(rm.sum(),self.problem.dimension))
            pop[rm,:-1]=np.clip(rv,lo,hi)
