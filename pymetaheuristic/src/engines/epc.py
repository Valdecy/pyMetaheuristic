...
class EPCEngine(PortedPopulationEngine):
    ...
    def _step_impl(self, state, pop):
        ...
        hr = float(state.payload.get("heat_radiation",1.0))*hd; cmf = mf*(1-t/T); evals=0
        ...
        for i in range(n):
            for j in range(n):
                if self._is_better(float(pop[j,-1]),float(pop[i,-1])):
                    ...
                    attraction = att*(pop[j,:-1]-pop[i,:-1])*b_val
                    thermal = cmf*np.random.normal(0,1,dim)*pop[i,:-1]
                    pos = np.clip(pop[i,:-1]+attraction+thermal, self._lo, self._hi)
                    fit = float(self.problem.evaluate(pos)); evals+=1
                    if self._is_better(fit, float(pop[i,-1])):
                        pop[i,:-1]=pos; pop[i,-1]=fit
                        operator_labels[i] = "epc.spiral_attraction_update" if float(np.linalg.norm(attraction)) >= float(np.linalg.norm(thermal)) else "epc.thermal_mutation_update"
        return pop, evals, {"heat_radiation":hr, "operator_labels": operator_labels}
