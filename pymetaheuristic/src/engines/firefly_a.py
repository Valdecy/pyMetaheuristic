"""pyMetaheuristic src — Firefly Algorithm Engine"""
...
class FIREFLY_AEngine(BaseEngine):
    ...
    def __init__(self, problem, config):
        ...
        self._a=float(p["alpha_0"]); self._b=float(p["beta_0"]); self._g=float(p["gama"])
    ...
    def step(self, state):
        ...
        for i in range(self._n):
            for j in range(self._n):
                if i!=j:
                    fi=pop[i,:-1]; fj=pop[j,:-1]
                    rij=np.sqrt(np.sum((fi-fj)**2))
                    li=pop[i,-1]*np.exp(-self._g*rij**2); lj=pop[j,-1]*np.exp(-self._g*rij**2)
                    if li>lj:
