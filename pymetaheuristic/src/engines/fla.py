        ...
        if tf < 0.9:
            ...
            if tdo < np.random.random():
                ...
                for i in range(nt12):
                    ...
                    attempted_labels[int(order[i])] = "fla.forward_diffusion_transfer"
                for i in range(nt12, n1):
                    ...
                    new_pos[order[i]] = np.clip(pos, self._lo, self._hi)
                    attempted_labels[int(order[i])] = "fla.source_fluid_diffusion"
                for i in range(n2):
                    pos  = best2 + dof * (np.random.random(dim) * self._span + self._lo)
                    new_pos[order[n1 + i]] = np.clip(pos, self._lo, self._hi)
                    attempted_labels[int(order[n1 + i])] = "fla.receiver_fluid_diffusion"
            else:
                ...
                for i in range(nt12):
                    ...
                    new_pos[order[n1 + i]] = np.clip(pos, self._lo, self._hi)
                    attempted_labels[int(order[n1 + i])] = "fla.reverse_diffusion_transfer"
            ...
            for i in range(n):
                ...
                attempted_labels[i] = "fla.equilibrium_exploitation_update"
