...
class GRASPEngine(RestartLocalSearchEngine):
    ...
    def _construct(self) -> tuple[np.ndarray, float, int]:
        ...
        pool = np.random.uniform(self._lo, self._hi, (pool_size, self.problem.dimension))
        ...
        chosen = int(np.random.choice(order[:rcl_size]))
        return pool[chosen].copy(), float(fit[chosen]), pool_size

    def step(self, state):
        start, start_fit, evals = self._construct()
        cand, cand_fit, extra, delta = self._local_search(start, start_fit=start_fit)
        evals += extra
