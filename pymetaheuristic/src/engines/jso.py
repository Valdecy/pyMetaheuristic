"""pyMetaheuristic src — Jellyfish Search Optimizer Engine"""
...
class JSOEngine(BaseEngine):
    ...
    def __init__(self, problem, config):
        ...
        self._n = int(p["jellyfishes"])
        self._eta=np.clip(float(p["eta"]),3.57,4); self._beta=float(p["beta"]); self._gamma=float(p["gamma"])
    ...
    def step(self, state):
        ...
        old=np.copy(pop); dim=self.problem.dimension
        mu=pop[:,:-1].mean(axis=0)
        rand=np.random.rand(); ct=abs((1-t/T)*(2*rand-1))
        if ct>=0.5:
            r1=np.random.rand(self._n,dim); r2=np.random.rand(self._n,dim)
            bm=np.tile(elite[:-1],(self._n,1))
            pop[:,:-1]=np.clip(pop[:,:-1]+r1*(bm-self._beta*r2*mu),lo,hi)
        else:
            r=np.random.rand()
            if r>1-ct:
                rv=np.random.rand(self._n,dim)
                pop[:,:-1]=np.clip(pop[:,:-1]+self._gamma*rv*(hi-lo),lo,hi)
            else:
                for i in range(self._n):
                    cands=[x for x in range(self._n) if x!=i]; k=np.random.choice(cands)
                    rv=np.random.rand(dim)
                    d=pop[k,:-1]-pop[i,:-1] if pop[i,-1]>=pop[k,-1] else pop[i,:-1]-pop[k,:-1]
                    pop[i,:-1]=np.clip(pop[i,:-1]+rv*d,lo,hi)
        pop[:,-1]=self._evaluate_population(pop[:,:-1]); evals+=self._n
        combined=np.vstack([old,pop]); combined=combined[combined[:,-1].argsort()]
