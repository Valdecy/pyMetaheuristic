"""pyMetaheuristic src — Self-Adaptive Bat Algorithm Engine"""
...
class SABAEngine(PortedPopulationEngine):
    """Self-Adaptive Bat Algorithm — bat search with adaptive loudness and pulse rates."""
    algorithm_id = "saba"
    algorithm_name = "Self-Adaptive Bat Algorithm"
    family = "swarm"
    ...
    _DEFAULTS = dict(population_size=100, min_loudness=0.9, max_loudness=1.0, min_pulse_rate=0.001, max_pulse_rate=0.1, tao_1=0.1, tao_2=0.1, min_frequency=0.0, max_frequency=2.0)
    ...
    def _step_impl(self, state, pop):
        ...
        for i in range(n):
            ...
            sol = best + 0.1 * np.random.normal(size=self.problem.dimension) * np.mean(loudness) * self._span if np.random.rand() < pulse[i] else pop[i, :-1] + velocities[i]
