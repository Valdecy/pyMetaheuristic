"""pyMetaheuristic src — Teaching Learning Based Optimization Engine"""
...
class TLBOEngine(BaseEngine):
    ...
    def step(self, state):
        ...
        evals=0
        teacher=pop[pop[:,-1].argsort()][0,:]
        mean=pop.mean(axis=0); tf=np.random.choice([1,2],size=self._n)
        ri=np.random.rand(self._n,1)
        ...
        br=pop[:,-1]<pop[idxl,-1]; dr=np.where(br,-1,1)
