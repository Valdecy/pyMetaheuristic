...
# ---------------------------------------------------------------------------
# README-synchronized public operator label overrides
# ---------------------------------------------------------------------------
# Generated from README.md algorithm table.  The static EvoMapX catalog is
# the source consumed by docs/UI helpers, while engine-native telemetry may
# still emit a subset during a specific run.  Keep labels_for_algorithm()
# consistent with the README operator column.
_README_OPERATOR_LABEL_OVERRIDES: dict[str, list[str]] = {
    'aaa': [
        'aaa.recombination',
        'aaa.selection',
        'aaa.adaptation_most_starving_colony_moves_toward',
...
try:
    ENGINE_OPERATOR_LABELS.update({k: list(v) for k, v in _README_OPERATOR_LABEL_OVERRIDES.items()})
except Exception:
    pass
try:
    _ENGINE_OPERATOR_LABEL_OVERRIDES.update({k: list(v) for k, v in _README_OPERATOR_LABEL_OVERRIDES.items()})
except Exception:
    pass

_prev_labels_for_algorithm_readme_sync = labels_for_algorithm
def labels_for_algorithm(algorithm_id: str) -> list[str]:  # type: ignore[override]
    aid = str(algorithm_id).lower().replace("-", "_")
    if aid in _README_OPERATOR_LABEL_OVERRIDES:
