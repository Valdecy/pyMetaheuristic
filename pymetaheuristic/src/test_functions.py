...
def three_bar_truss_constraints(variables_values: ArrayLike | None = None, P: float = 2.0, sigma: float = 2.0) -> List[float]:
    ...
    Candidate solutions generated by population methods may temporarily place
    both areas very close to zero.  The mathematical constraints then contain
    zero denominators, which used to emit NumPy divide-by-zero warnings.  Such
    points are simply infeasible, so this implementation returns a large finite
    violation for the affected constraints instead of dividing by zero.
    """
    x = _asarray(variables_values, [0.7886751346, 0.4082482905])
    ...
    if den12 <= eps:
        ...
        g1 = float((sqrt2*A1 + A2)*P/den12 - sigma)
        g2 = float(A2*P/den12 - sigma)

    if den3 <= eps:
        g3 = penalty
    else:
        g3 = float(P/den3 - sigma)
...
TEST_FUNCTIONS: Dict[str, Dict[str, str]] = {
    ...
    "rosenbrock": {"name": "Rosenbrock Valley", "domain": "usually [-5,10]^D", "optimum": "x*=1_D; f*=0"},
    ...
    "sphere": {"name": "Sphere / De Jong F1", "domain": "usually [-5.12,5.12]^D", "optimum": "x*=0_D; f*=0"},
